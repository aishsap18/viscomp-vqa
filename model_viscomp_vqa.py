...
class Answer_Generator():
    ...
    def build_model(self):
        ...
        # state_options = self.stacked_lstm_o.zero_state(self.batch_size, dtype=tf.float32)
        # for i in range(6):
        #     if i==0:
        #         # print("self.input_embedding_size: {}".format(self.input_embedding_size))
        #         options_emb_linear = tf.zeros([self.batch_size, self.input_embedding_size])
        #     else:
        #         # print("hello")
        #         tf.get_variable_scope().reuse_variables()
        #         options_emb_linear = tf.nn.embedding_lookup(self.embed_options_W, mc_answers[:,i-1])


        #     options_emb_drop = tf.nn.dropout(options_emb_linear, 1-self.drop_out_rate)
        #     options_emb = tf.tanh(options_emb_drop)

        #     output_options, state_options = self.stacked_lstm_o(options_emb, state_options)

        # options_state_drop = tf.nn.dropout(state_options, 1-self.drop_out_rate)
        # print("\nstate0: {} \n state1: {} \n".format(state_drop[0], state_drop[1]))

        # options_state_linear = tf.compat.v1.nn.xw_plus_b(tf.concat([options_state_drop[0], options_state_drop[1]], 1), 
        #     self.embed_options_state_W, self.embed_options_b)
        # options_state_emb = tf.tanh(options_state_linear)

        # bidirectional lstm options
        # _X = (mc_answers, )
        # tf.get_variable_scope().reuse_variables()
        # outputs_bilstm, state_fw, state_bw = tf.nn.static_bidirectional_rnn(self.lstm_multicell_o_fwd, self.lstm_multicell_o_bwd, 
        #     _X, dtype='float32')
        # options_state_bilstm = tf.concat([state_fw, state_bw], 0)

        # options_state_bilstm_drop = tf.nn.dropout(options_state_bilstm, 1-self.drop_out_rate)

        # # print("\nstate0: {} \n state1: {} \n".format(options_state_bilstm_drop[0], options_state_bilstm_drop[1]))
        # # options_state_linear = tf.compat.v1.nn.xw_plus_b(options_state_bilstm_drop, 
        # #     self.embed_options_state_W, self.embed_options_b)

        # options_state_linear = tf.compat.v1.nn.xw_plus_b(tf.concat([options_state_bilstm_drop[0], options_state_bilstm_drop[1]], 1), 
        #     self.embed_options_state_W, self.embed_options_b)
        # options_state_emb = tf.tanh(options_state_linear)
...
if __name__ == '__main__':
    ...
    checkpoint_path = checkpoint_path + params['checkpoint_path']
