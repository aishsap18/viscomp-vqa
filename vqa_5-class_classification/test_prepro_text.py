...
def test(model_path, dataset, data_train, data_test):
	...
	def get_embeddings(num, data):
		...
		for current_batch_start_idx in range(0,num,batch_size):
			...
			if method == 'bert':
				# print("\nmethod: {}\n".format(method))
				# print("data['text_embeddings']: {}".format(data['text_embeddings'].shape))
...
if __name__ == '__main__':
	...
	parser.add_argument('--checkpoint_model', required=True, help='enter checkpoint model path')
	parser.add_argument('--out_name', default='data_text_train.h5', help='enter output file name')
	...
	checkpoint_path = params['checkpoint_model']
	
	f = h5py.File(params['out_name'], "w")

	with tf.device('/gpu:'+str(0)):	
		dataset, data_train = get_train_data(method, input_json, input_ques_h5, input_bert_emb)
		dataset, data_test = get_test_data()
		num_train, desc_embed_train, num_test, desc_embed_test = test(checkpoint_path, dataset, data_train, data_test)
		...
		print("\n\nlen(desc_embed_train): {}".format(len(desc_embed_train)))
		# for item in desc_embed_train[:2]:
		# 	print(item)

		print("len(desc_embed_test): {}".format(len(desc_embed_test)))
